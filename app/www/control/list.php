...
class list_c extends Control {
    ...
    function compare_f() {
        ...
        $id = implode(',', $id);
        $rslist = $this->list_m->get_compare($id);
        $this->tpl->assign("rslist", $rslist); //dump($rslist);
        $this->tpl->display("list_compare.html");
    }
    ...
    function index_cate($cid) {
        ...
        $rs["jsjj"] = sys_format_content($rs["jsjj"]);
        $rs["jslx"] = sys_format_content($rs["jslx"]);
        $rs["jsys"] = sys_format_content($rs["jsys"]);
        $rs["dna"] = sys_format_content($rs["dna"]);
        $rs["aljx"] = sys_format_content($rs["aljx"]);
        ...
        if ($rs["if_index"]) {
            ...
            $total = $this->list_m->get_count();//$this->list_m->get_count_from_cate(); //取得总数量
            $this->tpl->assign("total", $total);
            $pagelist = $this->page_lib->page_www($pageurl, $total, true); //分页数组
            $this->tpl->assign("pagelist", $pagelist);
            $this->tpl->assign("rslist", $rslist);
            if ($_POST['ajax']) {
                $this->tpl->display($this->tplfile . "_ajax." . $this->tpl->ext);
            } else {
                $this->tpl->display($this->tplfile . "." . $this->tpl->ext);
            }
        }
