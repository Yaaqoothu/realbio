...
if (!function_exists("phpok_menu")) {
    ...
    function phpok_menu($id = 0, $cid = 0, $mid = 0) {
        ...
        foreach ($tmplist AS $key => $value) {
            if (!$value["link_html"])
                $value["link_html"] = $value["link"];
            if (!$value["link_rewrite"])
                $value["link_rewrite"] = $value["link"];
...
//mid：模块ID
//在模板中，调用可以直接编写： <!-- run:$menulist = phpok_menu($id,$cid,$mid) -->
if (!function_exists("phpok_menu2")) {

    function phpok_menu2($menuid=0,$id = 0, $cid = 0, $mid = 0) {
        $app = sys_init();
        $app->load_model("menu");
        $tmplist = $app->menu_m->get_all2();
        if (!$tmplist) {
            return false;
        }
        $rslist = array();
        ...
        $tree = new Tree();
        return $tree->list_to_tree($rslist, 'id', 'parentid', '_child',0,$menuid);
        //echo ('<div class="c_subNav">'.$tree->getNav($rslist).'<div style="clear:both"></div></div>');
    }
...
//浏览历史
    function getHis() {
        ...
        $his_id = $_COOKIE['his_id']?unserialize($_COOKIE['his_id']):'';
		if(is_array($his_id))   
{            
        foreach ($his_id as $key => $value) {
            $id[$key] = (int) $value;
        }
...
if (!function_exists("phpok_plugin")) {
    ...
    function phpok_plugin($plugin_identifier, $function = "phpok", $ext = "") {
        ...
        $app = sys_init();
        $app->plugin($plugin_identifier)->$function($ext);
    }
...
//function，执行函数
//ext，传递参数，数组或字符串，受插件影响，一般是传递字符串
if (!function_exists("getModuleField")) {

    function getModuleField($identifier) {
        $app = sys_init();
        $app->load_model("module");
        $data = $app->module_m->getFields($identifier);
        if($data){
            $options = explode("\n", $data['list_val']);
            $inputName = $data['input']==='checkbox'?$data['identifier'].'[]':$data['identifier'];
            foreach ($options as $value) {
                if(is_array($_POST[$data['identifier']]) && in_array($value, $_POST[$data['identifier']])){
                    $checked = ' checked="checked"';
                }else{
                    $checked ='';
                }
                $input[]='<input type="'.$data['input'].'" value="'.$value.'" id="'.md5($value).'" name="'.$inputName.'"'.$checked.' /><label  for="'.md5($value).'">'.$value.'</label>';
            }
            return $input;
        }
    }
